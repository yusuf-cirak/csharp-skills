...
- Validation goes in the factory method. Return a monad type (`Result<T>` / `Option<T>` — from YC.Monad if available, otherwise the codebase's existing equivalent) when validation can fail. See `../csharp-coding-standards/references/monads.md`.
- Use **value objects** for complex primitives. Base type: `../csharp-coding-standards/references/value-object-base.md`.
...
The concrete slice skeleton (Endpoint/Request/Validator/Handler/Response) and FluentValidation conventions live in `aspnetcore-web-api` — this skill only fixes *where* slices sit in the module layout.

## Related skills

- `csharp-language` — record/VO/DU/monad language patterns used by domain models.
- `aspnetcore-web-api` — slice skeleton, handlers, endpoints.
- `aspnetcore-input-validation` — request DTO limits and validators.
- `aspnetcore-production-hardening` — multi-tenancy, outbox, EF hardening for the infra layer.
- `csharp-testing` — architecture tests enforce these layer boundaries; unit tests for domain factories.
